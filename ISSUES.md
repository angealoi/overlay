...
- **결론**: **결함 아님.** stable `HitObjectManager_LoadSave.cs:815-817`이 우리와 **byte 단위로 동일**하게 `y = (int)Max(0, Min(512, y))` 후 `verticalFlip ? 384 - y : y`를 한다. 즉 y∈(384,512]에서 음수가 되는 건 stable의 동작 그대로이며 우리가 1:1이다. 초기 분석의 오탐 (stable 대조로 확정)
...
- **검증**: 적대적 리뷰로 정상 선분 항등·잔여 NaN 지점 0 확인. 전부 degenerate/에일리언 맵 한정, 정상 맵 무변화
